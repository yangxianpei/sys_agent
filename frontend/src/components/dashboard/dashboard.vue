...
import {get_agent,get_model,usage_count,usage} from '../../api/dashboard'
...
const fetchUsageData = async () => {
  ...
  try {
    ...
    // 获取调用次数数据
    const countRes = await usage_count(params)
    if (countRes.code === 200) {
      updateCallCountChart(countRes.data)
      // 累计调用次数（使用显式遍历避免 unknown 类型问题）
      const dk: 'agent' | 'model' = (filters.value.agent ? 'agent' : 'model')
      let calls = 0
      const dayList = Object.values(countRes.data || {}) as Array<any>
    ...
    // 获取Token使用量数据
    const statsRes = await usage(params)
    if (statsRes.code === 200) {
      updateTokenUsageChart(statsRes.data)
      // 累计Token（使用显式遍历避免 unknown 类型问题）
      const dk: 'agent' | 'model' = (filters.value.agent ? 'agent' : 'model')
      let tokens = 0
      const dayList = Object.values(statsRes.data || {}) as Array<any>
      for (const day of dayList) {
