  ...
  if (uploadFile.raw) {
    ...
    try {
      ...
      if (response.code === 200) {
        formData.logo_url = import.meta.env.VITE_API_BASE_URL+ response.data.url
...
const saveAgent = async () => {
  try {
    ...
    if (isEditing.value) {
      ...
      const response = await modfy_agent(updateData)
...
const validateIdMatching = () => {
  ...
  if (formData.mcp_ids.length > 0) {
    const mcpOptionsIds = mcpOptions.value.map(m => m.mcp_id)
...
const initializeData = async () => {
  ...
  try {
    const d=await get_agent_all()
    if( d.code ==200){
      toolOptions.value= d.data.tools
                  ...
                  >
                    ...
                    >
                      <div class="custom-option">
                        ...
                        <span class="option-name">{{ llm.model }}</span>
              ...
              <div v-show="collapseItems.tools" class="section-content">
                <el-form-item label="选择工具">
                  ...
                  >
                    ...
                    >
                      <div class="custom-option">
                        <img :src="tool.logo || `/public/${tool.name}.png`" class="option-logo" :alt="tool.name" />
              ...
              <div v-show="collapseItems.mcp" class="section-content">
                <el-form-item label="MCP服务">
                  ...
                  >
                    ...
                    >
                      <div class="custom-option">
                        <img :src="mcp.logo || '/src/assets/mcp.svg'" class="option-logo" :alt="mcp.name" />
