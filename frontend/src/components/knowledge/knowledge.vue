...
import {create_knowledge,knowledge_list,knowledge_del,modify_knowledge} from '../../api/knowledge'
...
const fetchKnowledges = async () => {
  loading.value = true
  try {
         const response = await knowledge_list()
     if (response.code === 200) {
       knowledges.value = response.data.map((item: KnowledgeResponse) => ({
         id: item.id,
         name: item.name,
...
const confirmDelete = async () => {
  ...
  try {
    ...
    const response = await knowledge_del(knowledgeToDelete.value.id)
    if (response.code === 200) {
...
const handleCreate = async () => {
  ...
  try {
    const response = await create_knowledge({name,description:desc})
    debugger
    if (response.code === 200) {
...
const handleEdit = async () => {
  ...
  try {
    const updateData = {
      id: editForm.value.knowledge_id,
      name: editForm.value.knowledge_name,
      description: editForm.value.knowledge_desc
    }
    
    const response = await modify_knowledge(updateData)
    if (response.code === 200) {
