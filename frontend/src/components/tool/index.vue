...
//   type ToolResponse 
// } from '../../apis/tool'
import {Upload,regist_tool,get_tool_list,del_tool,modify_tool} from '../../api/tool'
import {type Atool} from '../../api/tool'
import { useUserStore } from '../../store/user'

// 工具类型定义
interface Tool extends Atool {
  user_id: string
  tool_id:string,
  type:string,
  created_at?: string
...
interface CreateToolForm {
  ...
  description: string
  logo_url: string
  openai_schema?: any  // OpenAPI Schema
  auth_config?: {
    type: '' | 'bearer' | 'basic'
    token?: string
  }
...
const exampleSchemas = [
  {
    ...
    "servers": [{"url": "https://uapis.cn/api/v1/misc"}],
    "paths": {
        "/weather": {
            "get": {
                "summary": "获取天气",
...
const parseOpenAPISchema = () => {
  try {
    ...
    createForm.value.openai_schema = schema
  } catch (error) {
...
const handleLogoUpload = async (event: Event) => {
  ...
  try {
    ...
    if (response.code === 200) {
      ...
      const uploadedUrl = import.meta.env.VITE_API_BASE_URL+ response.data.url as any as string
...
const handleEditLogoUpload = async (event: Event) => {
  ...
  try {
    const response = await Upload(file)
    if (response.code === 200) {
      const uploadedUrl = import.meta.env.VITE_API_BASE_URL+response.data.url
...
const parseEditOpenAPISchema = () => {
  try {
    ...
    editForm.value.openai_schema = schema
  } catch (error) {
    ElMessage.error('OpenAPI Schema 格式错误')
  ...
  try {
   ...
   let data= response.data.map(item=>{
    ...
    if (response.code === 200) {
      ...
      ElMessage.error(response.message || '获取工具列表失败')
    }
  } catch (error) {
...
const handleCreateTool = async () => {
  try {
    ...
    const requestData:any = {
      name: createForm.value.display_name,
      description: createForm.value.description,
      logo: createForm.value.logo_url,
      openai_schema: createForm.value.openai_schema
    }
    ...
    if (authMethod.value && authMethod.value !== '') {
     let  auth_config = {
      ...
      requestData['auth_config']=auth_config
    }
    const response = await regist_tool(requestData)
    if (response.code === 200) {
...
const handleEditTool = async () => {
  try {
    ...
    const requestData:any = {
      ...
      name: editForm.value.display_name,
      description: editForm.value.description,
      logo: editForm.value.logo_url,
      openai_schema: editForm.value.openai_schema || undefined
    }
    ...
    const response = await modify_tool(requestData)
    if (response.code === 200) {
      ...
      ElMessage.error(response.message || '更新工具失败')
    }
  } catch (error) {
...
const confirmDeleteTool = async () => {
  ...
  try {
    const response = await del_tool( toolToDelete.value.tool_id)
    if (response.code === 200) {
      ...
      ElMessage.error(response.message || '删除工具失败')
    }
  } catch (error) {
...
const openEditDrawer = (tool: Tool) => {
  ...
  editForm.value = {
    ...
    auth_config:
      (tool.auth_config as UpdateToolForm['auth_config']) ?? { type: '' }
  }
  
  // 设置头像预览
  editLogoPreview.value = tool.logo || ''
  
  // 设置 OpenAPI Schema
  if (tool.openai_schema) {
    editSchemaContent.value = JSON.stringify(tool.openai_schema, null, 2)
    parseEditOpenAPISchema()
  } else {
    ...
    <div class="tool-container" v-loading="loading">
      ...
      <div class="tool-list" v-if="tools.length > 0">
        ...
        >
          ...
          <div class="col-type">
            <span v-if="tool.type == '1'" class="type-badge system">
          ...
          <div class="col-time">
            <span class="time-text">{{ formatRelativeTime(tool.created_at) }}</span>
          </div>
