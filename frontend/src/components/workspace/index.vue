...
import {session_list,del_session_by_id} from '../../api/workspace'
...
// 获取会话列表
const fetchSessions = async (refresh='') => {
  try {
    loading.value = true
...
const deleteSession = async (sessionId: string, event: Event) => {
  ...
  try {
    const response = await del_session_by_id(sessionId)
    if (response.code === 200) {
  ...
  <div class="workspace-container">
    ...
    <div class="workspace-nav">
      <div class="nav-left">
        ...
        <div class="nav-links">
          <img src="../../assets/mylogo.svg" alt="hello_agent" class="brand-logo-img" />
        </div>
      </div>
      <div class="nav-right">
        <!-- 用户信息区域 -->
        <div class="user-info">
          <el-dropdown
            popper-class="workspace-nav-user-dropdown"
...
<style lang="scss">
.workspace-nav-user-dropdown.el-popper {
