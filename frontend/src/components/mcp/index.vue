...
import {regist_mcp_api,get_mcp_list,del_mcp_list,modify_mcp_api} from '../../api/mcp'
import { useUserStore } from '../../store/user'

type MCPServer = any
type MCPServerTool=any
...
/** 上传时动态带 token：普通对象只在 setup 求值一次，会拿不到登录后的 token */
...
const handleLogoUploadSuccess = (response: any) => {
  ...
  if (imageUrl) {
    formData.value.logo_url = import.meta.env.VITE_API_BASE_URL+imageUrl
...
const handleLogoUploadError = (err: any) => {
...
const handleEdit = (server: MCPServer) => {
  ...
  formData.value = {
    ...
    imported_config: server.server_config
  }
  
  // 直接使用服务器的imported_config
  if (server.server_config) {
    configString.value = JSON.stringify(server.server_config, null, 2)
  } 
}
...
const handleSubmit = async () => {
  ...
  try {
    if (editingServer.value) {
      ...
      const updateData = {
        mcp_id: editingServer.value.mcp_id,
        mcp_name: formData.value.server_name,
        logo: formData.value.logo_url,
        mcpServers: JSON.parse(configString.value).mcpServers
      }
      const response = await modify_mcp_api(updateData)
      ...
      const response = await regist_mcp_api(submitData)
      if (response.code === 200) {
...
const confirmDelete = async () => {
  ...
  formLoading.value = true
  try {
    const response = await del_mcp_list(deletingServer.value.mcp_id)
    if (response.code === 200) {
...
const viewTools = (server: MCPServer) => {
  selectedServerTools.value = server.tools || []
...
const getMcpList=async ()=>{
    const d = await get_mcp_list()
     servers.value=d.data.map((item:any)=>{
        return {
...
const uploadUrl = `${import.meta.env.VITE_API_BASE_URL}/api/v1/upload`
</script>
    ...
    <div class="server-list">
      <el-table v-if="servers.length > 0" :data="servers || []" style="width: 100%">
        ...
        <el-table-column prop="type" label="连接类型" width="110" align="center">
          <template #default="{ row }">
            <el-tag :type="row.connect_type === 'sse' ? 'primary' : 'success'">
          ...
          <template #default="{ row }">
            <div class="tools-count">
              <el-button 
                ...
                :icon="Tools"
                size="small"
                :disabled="!row.tools || row.tools.length === 0"
                round
                 @click="viewTools(row)"
              >
                {{ row.tools?.length || 0 }} 个工具
              </el-button>
            </div>
          </template>
        </el-table-column>
