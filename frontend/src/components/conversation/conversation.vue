...
import {agent_all_list} from '../../api/agent'
import {diolog_list,create_dialog,del_dialog} from '../../api/dialog'
...
const fetchAgents = async () => {
  try {
    ...
    if (response.code === 200) {
      ...
      ElMessage.error(`获取智能体列表失败: ${response.message}`)
    }
  } catch (error) {
...
const setHistory = (history: any[] | undefined) => {
    const h: any[] = []
    if (!history || !Array.isArray(history)) {
        historyChatStore.setHistory([])
        return
    }
    history.forEach((item) => {
        const query = item.query
        const answe = item.answer
        const chatMsg = {
            personMessage: { content: query },
            aiMessage: { content: answe },
            eventInfo: []
        }
        h.push(chatMsg)
    })
    historyChatStore.setHistory(h)
}
// 获取对话列表
const fetchDialogs = async () => {
  try {
    loading.value = true
    const response = await diolog_list()
    if (response.code === 200) {

        debugger
      dialogs.value = response.data.map((dialog: any) => {
        const processedDialog = {
          ...
          logo: dialog.logo || 'https://via.placeholder.com/40x40/3b82f6/ffffff?text=AI',
      ...
      if (dialogs.value.length > 0 ) {
        const firstDialog = dialogs.value[0] as any
        console.log('立即自动打开第一个会话:', firstDialog.dialogId, firstDialog.name)

        setHistory(firstDialog.history)
...
const createDialog = async () => {
  ...
  if (agent) {
    try {
      ...
      const response = await create_dialog(dialogData)
      if (response.code === 200) {
        ...
        if (dialogId) {
          ...
          historyChatStore.logo = agent.logo || 'https://via.placeholder.com/40x40/3b82f6/ffffff?text=AI'
...
const deleteDialog = async (dialogId: string) => {
  ...
  try {
    const response = await del_dialog(dialogId)
    if (response.code === 200) {
      ElMessage({
        message: '会话删除成功',
        type: 'success',
      ...
      ElMessage.error(`删除会话失败: ${response.message}`)
    }
  } catch (error) {
...
const selectDialog = (dialogId: string) => {
  ...
  setHistory((dialog as any).history)
