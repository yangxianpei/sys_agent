...
import {sendMessage} from '../../api/workspace'
...
const userAvatar = computed(() =>  defaultUserAvatar)
...
const handleEventStatus = (parsedData: any) => {
  ...
  if (status === 'START') {
    ...
    if (existingEventIndex === -1) {
      ...
      lastChat.eventInfo[existingEventIndex].message = messages || "处理中..."
    }
    // 设置有活跃事件
    ...
    if (existingEventIndex !== -1) {
      lastChat.eventInfo[existingEventIndex].status = status
      if (messages) {
        lastChat.eventInfo[existingEventIndex].message = messages
      }
    } else {
      // 如果没有找到对应的事件，创建一个新事件
      lastChat.eventInfo.push({
        event_type: eventId,
        message: messages || (status === 'END' ? "已完成" : "处理出错"),
        status: status,
        show: false // 默认折叠
      })
    }
      ...
      abortCtrl.value = sendMessage(
        ...
        (msg: any) => {
          ...
          try {
            ...
            // 日志须在 debugger 之前，否则断点暂停时 console 尚未执行
...
console.log(historyChatStore.chatArr);
