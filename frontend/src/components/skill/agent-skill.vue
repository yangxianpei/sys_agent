...
import {reagisterAPI,list_agent_skill,del_agent_skill,file_add,file_update} from '../../api/skill'
// 响应式数据
...
const fetchSkills = async () => {
  loading.value = true
  try {
    const response = await list_agent_skill()
    if (response.code === 200) {
      skills.value = response.data || []
    } else {
      ElMessage.error(response.message || '获取 Skill 列表失败')
    }
  } catch (error) {
...
const handleCreateSkill = async () => {
  ...
  try {
    ...
    if (response.code === 200) {
      ...
      ElMessage.error(response.message || '创建 Skill 失败')
    }
  } catch (error) {
...
const handleDeleteSkill = async (skill: AgentSkill, event?: Event) => {
  ...
  try {
    const response = await del_agent_skill( skill.id)
    if (response.code === 200) {
      ElMessage.success('Skill 删除成功')
      fetchSkills()
    } else {
      ElMessage.error(response.message || '删除 Skill 失败')
    }
  } catch (error) {
...
const saveFileContent = async () => {
  ...
  try {
    ...
    const response = await file_update({
...
const handleAddFile = async () => {
  ...
  try {
    ...
    if (response.code === 200) {
      ...
      ElMessage.error(response.message || '添加文件失败')
    }
  } catch (error: any) {
    ...
    <div class="skill-container" v-loading="loading">
      ...
      <div class="skill-list" v-if="skills.length > 0">
        ...
        >
          ...
          <div class="col-time">
            <span class="time-text">{{ formatRelativeTime(skill.created_at) }}</span>
          </div>
              ...
              <div class="skill-info-panel">
                ...
                <div class="info-meta">
                  <span>
                    ...
                    创建于 {{ formatTime(currentSkill?.created_at) }}
                  </span>
                </div>
              </div>
            </div>
